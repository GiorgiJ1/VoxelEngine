...
use voxel_core::{
    export_gltf_glb, export_obj_mtl, greedy_mesh, load_chunk, raycast_chunk, save_chunk, Chunk, MeshData, Voxel, CHUNK_SIZE, line_voxels,
};
...
impl Gpu {
        ...
        match button {
            ...
            MouseButton::Left => {
                if state == ElementState::Pressed {
                    ...
                    self.continue_paint_stroke(x, y);
                } else {
                    self.painting = false;
                    self.last_paint_cell = None;
                }
    ...
    /// Called on left-button press and on every cursor move while painting.
    /// Finds this frame's target cell, and if it isn't grid-adjacent to
    /// the last cell painted (the mouse jumped more than one cell since
    /// last frame), fills in every cell along the line between them so
    /// the stroke has no gaps.
    fn continue_paint_stroke(&mut self, x: f64, y: f64) {
        ...
        let target_cell = match self.paint_mode {
            PaintMode::Add => hit.place_at,
            PaintMode::Replace | PaintMode::Remove => Some(hit.voxel),
        };
        ...
        match self.last_paint_cell {
            Some(prev) if prev != target_cell => {
                for cell in line_voxels(prev, target_cell) {
                    self.paint_single_cell(cell);
                }
            }
            Some(_) => {}
            None => self.paint_single_cell(target_cell),
        }

        self.last_paint_cell = Some(target_cell);
    }

    /// Applies the current paint mode to exactly one cell. Snapshots for
    /// undo at most once per stroke (on the first cell that actually
    /// changes), not once per voxel, so a whole drag stroke undoes as a
    /// single action. Marks `mesh_dirty` instead of rebuilding
    /// immediately -- a fast stroke can touch many cells in one frame,
    /// and `render()` rebuilds once per frame if anything changed.
    fn paint_single_cell(&mut self, cell: IVec3) {
        let size = CHUNK_SIZE as i32;
        if cell.x < 0 || cell.y < 0 || cell.z < 0 || cell.x >= size || cell.y >= size || cell.z >= size {
            return;
        }

        let new_voxel = match self.paint_mode {
            PaintMode::Add | PaintMode::Replace => Voxel::new(self.current_material),
            PaintMode::Remove => Voxel::EMPTY,
        };

        let (cx, cy, cz) = (cell.x as usize, cell.y as usize, cell.z as usize);
        let current = self.chunk.get(cx, cy, cz);
        ...
        self.chunk.set(cx, cy, cz, new_voxel);
                        ...
                        ui.horizontal_wrapped(|ui| {
                            for mat in &materials_edit {
                                ...
                                if is_selected {
                                    ...
                                    ui.painter().rect_stroke(rect, 4.0, egui::Stroke::new(2.0, blue_accent));
